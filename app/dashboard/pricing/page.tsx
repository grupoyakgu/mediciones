...
export default function PricingListPage() {
  ...
  if (loading) {
    return (
      <div>
        <h1 style={{ margin: '0 0 1.5rem', fontSize: '1.5rem', fontWeight: 700, color: '#0f172a' }}>Pricing Projects</h1>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: 'white', borderRadius: '14px', padding: '2rem', maxWidth: '420px', width: '90%', boxShadow: '0 20px 60px rgba(0,0,0,.2)' }}>
            ...
            <p style={{ margin: '0 0 1.5rem', fontSize: '.9rem', color: '#475569' }}>
              <strong>{confirmDelete.name}</strong> and all its results and alerts will be permanently deleted. This cannot be undone.
            </p>
      ...
      {showNew && (
        <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', padding: '1.25rem', marginBottom: '1.25rem', display: 'flex', gap: '.75rem' }}>
          <input
            ...
            placeholder="Pricing project name…"
            value={newName}
          ...
          <button
            ...
            style={{ padding: '.5rem 1rem', background: creating ? '#94a3b8' : '#0f172a', color: 'white', border: 'none', borderRadius: '6px', fontSize: '.875rem', fontWeight: 500, cursor: creating ? 'not-allowed' : 'pointer' }}
          >
          ...
          {projects.map(p => (
            ...
            >
              <button
                ...
                title="Delete pricing project"
              >✕</button>
              <div style={{ display: 'flex', alignItems: 'center', gap: '.75rem', marginBottom: '.75rem' }}>
                <div style={{ width: '40px', height: '40px', borderRadius: '10px', background: '#f0fdf4', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.1rem', fontWeight: 700, color: '#16a34a', flexShrink: 0 }}>
                  $
                </div>
