...
export default function BoqTable({ projectId }: { projectId: string }) {
  ...
  const th = (clickable = false): React.CSSProperties => ({
  ...
  if (items.length === 0) {
    return (
      <div style={{ ...card, textAlign: 'center', padding: '3rem 2rem', color: '#94a3b8', border: '2px dashed #e2e8f0' }}>
        ...
        <p style={{ margin: 0 }}>No BOQ loaded for this project. Go to <a href="settings" style={{ color: '#2563eb', fontWeight: 500 }}>Settings</a> to upload a BOQ file.</p>
      </div>
    )
  }
    ...
    <div>
      ...
      <div style={{ ...card, padding: '1rem 1.5rem' }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '.75rem', alignItems: 'center' }}>
          ...
          <input style={{ ...inp, width: '110px' }} placeholder="Max total (€)" value={maxPrice} onChange={e => setMaxPrice(e.target.value)} type="number" />
          <label style={{ display: 'flex', alignItems: 'center', gap: '.4rem', fontSize: '.875rem', color: '#475569', cursor: 'pointer', whiteSpace: 'nowrap' }}>
            <input type="checkbox" checked={showSummary} onChange={e => setShowSummary(e.target.checked)} style={{ width: '15px', height: '15px', cursor: 'pointer' }} />
            Chapter summary
          </label>
        </div>
      </div>
